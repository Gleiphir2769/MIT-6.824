...
func (tw *TimeWheel) getPosAndCircle(delay time.Duration) (pos int, circle int) {
	circle = int(delay.Milliseconds()) / int(tw.interval.Milliseconds()) / tw.slotNum
	pos = (tw.currentPos + int(delay.Milliseconds())/int(tw.interval.Milliseconds())) % tw.slotNum
	return circle, pos
}
